...
<div class="container-fluid my-4">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <div>
            <h1 class="mb-1">Edit Performance Review</h1>
            <p class="text-muted mb-0">{{ $performanceReview->employee->name }}</p>
        </div>
        <a href="{{ route('staff.performance.show', $performanceReview) }}" class="btn btn-outline-secondary">
    ...
    <form method="POST" action="{{ route('staff.performance.update', $performanceReview) }}" class="card shadow-sm">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-4">
                    ...
                    <select name="review_period" id="review_period" class="form-select @error('review_period') is-invalid @enderror" required>
                        <option value="annual" {{ old('review_period', $performanceReview->review_period) === 'annual' ? 'selected' : '' }}>Annual</option>
                        <option value="quarterly" {{ old('review_period', $performanceReview->review_period) === 'quarterly' ? 'selected' : '' }}>Quarterly</option>
                        <option value="probation" {{ old('review_period', $performanceReview->review_period) === 'probation' ? 'selected' : '' }}>Probation</option>
                    </select>
                            ...
                            @for ($i = 1; $i <= 5; $i++)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input @error($field) is-invalid @enderror" type="radio"
                                           ...
                                           value="{{ $i }}" {{ old($field, $performanceReview->$field) == $i ? 'checked' : '' }} required>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <textarea name="strengths" id="strengths" rows="3" class="form-control @error('strengths') is-invalid @enderror">{{ old('strengths', $performanceReview->strengths) }}</textarea>
                ...
                <div class="col-md-6">
                    ...
                    <textarea name="areas_for_improvement" id="areas_for_improvement" rows="3" class="form-control @error('areas_for_improvement') is-invalid @enderror">{{ old('areas_for_improvement', $performanceReview->areas_for_improvement) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <textarea name="comments" id="comments" rows="3" class="form-control @error('comments') is-invalid @enderror">{{ old('comments', $performanceReview->comments) }}</textarea>
