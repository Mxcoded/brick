...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="fw-bold display-5 text-primary">
            <i class="fas fa-calendar-plus me-2"></i>Add Event Day for Order #{{ $order->order_id }}
        </h1>
            ...
            <form action="{{ route('banquet.orders.store-day', $order->order_id) }}" method="POST" id="eventForm">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="event_description" id="event_description" class="form-control" rows="3"
                              placeholder="Enter a brief description of the event">{{ old('event_description') }}</textarea>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="event_type" id="event_type" class="form-select" required>
                            @foreach ($eventTypes as $type)
                                <option value="{{ $type }}" {{ old('event_type', 'Other') === $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="d-grid">
                    <button type="submit" class="btn btn-primary btn-lg">
                        <i class="fas fa-calendar-check me-2"></i>Add Event Day
                    </button>
                </div>
            </form>
        </div>
