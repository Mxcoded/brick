    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-header bg-white py-3 px-4 d-flex flex-wrap justify-content-between align-items-center gap-3 rounded-top-4">
            <h1 class="h4 mb-0 fw-bold" style="color: var(--theme-heading);">
                <i class="fas fa-edit me-2" style="color: var(--theme-primary);"></i>Edit Testimonial
            </h1>
            ...
            <form action="{{ route('website.admin.testimonials.update', $testimonial) }}" method="POST">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="guest_name" class="form-control @error('guest_name') is-invalid @enderror" value="{{ old('guest_name', $testimonial->guest_name) }}" required>
                        @error('guest_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Email (Optional)</label>
                        <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $testimonial->email) }}" placeholder="guest@example.com">
                        @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Rating <span class="text-danger">*</span></label>
                        <input type="number" name="rating" class="form-control @error('rating') is-invalid @enderror" min="1" max="5" value="{{ old('rating', $testimonial->rating) }}" required>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="type" id="testimonial_type" class="form-select @error('type') is-invalid @enderror" required>
                            @foreach (\Modules\Website\Models\Testimonial::TYPES as $t)
                                <option value="{{ $t }}" {{ old('type', $testimonial->type) === $t ? 'selected' : '' }}>{{ ucfirst($t) }}</option>
                            @endforeach
                        </select>
                    ...
                    @foreach ($categories as $field => $label)
                        <div class="col-md-4 col-lg">
                            ...
                            <input type="number" name="{{ $field }}" class="form-control" min="1" max="5" value="{{ old($field, $testimonial->{$field}) }}" placeholder="1-5">
                    ...
                    <div class="col-12" id="stay_fields">
                        ...
                        <input type="text" name="stay_type" class="form-control @error('stay_type') is-invalid @enderror" value="{{ old('stay_type', $testimonial->stay_type) }}" placeholder="e.g. Deluxe Room, Presidential Suite">
                    ...
                    <div class="col-md-6 d-none" id="restaurant_fields">
                        ...
                        <input type="text" name="dining_venue" class="form-control @error('dining_venue') is-invalid @enderror" value="{{ old('dining_venue', $testimonial->dining_venue) }}" placeholder="e.g. Sky Restaurant, Pool Bar">
                        @error('dining_venue')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-6 d-none" id="event_fields">
                        <label class="form-label fw-semibold">Event Name</label>
                        <input type="text" name="event_name" class="form-control @error('event_name') is-invalid @enderror" value="{{ old('event_name', $testimonial->event_name) }}" placeholder="e.g. New Year Gala, Wedding Reception">
                        @error('event_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Location / Branch</label>
                        <input type="text" name="location" class="form-control @error('location') is-invalid @enderror" value="{{ old('location', $testimonial->location) }}" placeholder="e.g. Asokoro, Abuja">
                        @error('location')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Guest Image (URL)</label>
                        <input type="text" name="guest_image" class="form-control @error('guest_image') is-invalid @enderror" value="{{ old('guest_image', $testimonial->guest_image) }}" placeholder="https://example.com/image.jpg">
                        @error('guest_image')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-6">
                        <div class="form-check form-switch mt-4">
                            <input type="checkbox" name="approved" class="form-check-input" id="approved" value="1" {{ old('approved', $testimonial->approved) ? 'checked' : '' }}>
