        ...
        <div class="col-lg-9">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white py-4 px-4 border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('website.rooms.index') }}" class="btn btn-sm btn-primary rounded-pill px-3 shadow-sm">
                ...
                <div class="card-body p-0">
                                        ...
                                        <tr>
                                            ...
                                            <td class="ps-4">
                                                <div class="d-flex align-items-center">
                                                    @if($booking->room && $booking->room->image_url)
                                                        <img src="{{ $booking->room->image_url }}" class="rounded me-3" style="width: 50px; height: 50px; object-fit: cover;">
                                                    @else
                                                    ...
                                                    @endif
                                                    <div>
                                                        <span class="d-block fw-bold text-dark">{{ $booking->room->name ?? 'Room Deleted' }}</span>
                                                        <span class="small text-muted">Ref: #{{ $booking->booking_reference }}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div class="d-flex flex-column small">
                                                    <span><i class="fas fa-calendar-alt text-muted me-1"></i> {{ \Carbon\Carbon::parse($booking->check_in_date)->format('M d') }} - {{ \Carbon\Carbon::parse($booking->check_out_date)->format('M d, Y') }}</span>
                                                    @php
                                            ...
                                            {{-- Amount --}}
                                            <td>
                                                <div class="fw-bold text-dark">₦{{ number_format($booking->total_amount) }}</div>
                                                <span class="badge {{ $booking->payment_status == 'paid' ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning' }} rounded-pill px-2" style="font-size: 0.7rem;">
                                            ...
                                            <td>
                                                ...
                                                @elseif($booking->status === 'checked_in')
                                                    <span class="badge bg-primary-subtle text-primary border border-primary px-2 py-1 rounded-pill">Checked In</span>
                                                @elseif($booking->status === 'cancelled')
                                                    <span class="badge bg-danger-subtle text-danger border border-danger px-2 py-1 rounded-pill">Cancelled</span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="text-end pe-4">
                                                @if($booking->status === 'pending' || $booking->status === 'confirmed')
                                                    <form action="{{ route('guest.bookings.cancel', $booking->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to cancel this booking?');">
                                                        @csrf
                                                ...
                                                @else
                                                    <a href="{{ route('website.contact') }}" class="btn btn-sm btn-outline-secondary rounded-pill px-3">
                                                        Support
                                                    </a>
                                                @endif
                                            </td>
                    ...
                    @else
                        <div class="text-center py-5">
                            ...
                            <a href="{{ route('website.rooms.index') }}" class="btn btn-primary rounded-pill px-4 shadow-sm">
                                Find a Room
                            </a>
                        </div>
                    @endif
