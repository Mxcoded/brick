...
<div class="container py-4">
    <div class="card shadow border-0">
        ...
        <div class="card-body">
            ...
            @php
                $isPreferredRoomTaken = \Modules\Frontdeskcrm\Models\Registration::where('room_id', $booking->room_id)
            ...
            @if($isPreferredRoomTaken)
                <div class="alert alert-warning border-start border-4 border-warning shadow-sm">
                    ...
                    <strong>Attention:</strong> The guest preferred <strong>{{ $booking->room->name }}</strong>, but it is currently occupied (perhaps an overstay). Please allocate a different room below.
                </div>
            @else
                <div class="alert alert-info border-start border-4 border-info shadow-sm">
                    <i class="fas fa-info-circle me-2"></i>
                    The guest selected <strong>{{ $booking->room->name }}</strong>. You can confirm this or move them to another room.
                </div>
            @endif
            ...
            <form action="{{ route('frontdesk.bookings.process', $booking->booking_reference) }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="room_id" class="form-select form-select-lg {{ $isPreferredRoomTaken ? 'is-invalid' : '' }}">
                            ...
                            <option value="{{ $booking->room_id }}" {{ !$isPreferredRoomTaken ? 'selected' : '' }} 
                                ...
                                {{ $booking->room->name }} (Guest Preference) {{ $isPreferredRoomTaken ? '[OCCUPIED]' : '' }}
                            </option>
                            ...
                            <optgroup label="Available Rooms">
                                @foreach(\Modules\Website\Models\Room::where('status', 'available')->where('id', '!=', $booking->room_id)->get() as $room)
                                    ...
                                    @php
                                        $isOccupied = \Modules\Frontdeskcrm\Models\Registration::where('room_id', $room->id)
                                    ...
                                    @unless($isOccupied)
                                        <option value="{{ $room->id }}">
                                            {{ $room->name }} ({{ $room->capacity }} Guests) - ₦{{ number_format($room->price) }}
                                        </option>
                                    @endunless
                                @endforeach
                            </optgroup>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control bg-light" value="{{ $booking->check_out_date->format('M d, Y') }}" readonly>
                    </div>

                    <div class="col-md-12">
                        <label class="form-label fw-bold">Guest ID / Passport Number <span class="text-danger">*</span></label>
                        <input type="text" name="identification_number" class="form-control" required placeholder="Enter ID from physical document">
                    </div>
                </div>
