...
<div class="max-w-4xl mx-auto p-6">
    <div class="bg-glass-bg backdrop-blur-md rounded-xl border border-glass-border shadow-2xl overflow-hidden text-base-white">
        ...
        <div class="bg-gradient-to-r from-green-600 to-indigo-700 px-6 py-4">
            ...
            <p class="text-primary-light text-sm">Use this form for immediate check-in and to complete all booking details.</p>
        </div>
        
        <form 
            {{-- Initialize as NOT a Guest Draft, enabling all Agent features --}}
        ...
        >
            ...
            {{-- Explicitly set to 0 to trigger full validation in StoreRegistrationRequest --}}
            <input type="hidden" name="is_guest_draft" value="0"> 
            
            {{-- Search Bar (Visible for Agents only) --}}
            <div x-show="currentStep === 1" class="bg-white/10 rounded-lg p-4 border border-glass-border mb-6">
            ...
            <div class="flex justify-between items-center text-sm font-medium mb-6">
                ...
                <span :class="{'text-primary-light': currentStep >= 2, 'text-base-white/50': currentStep < 2}">2. Booking Details</span>
                <div class="h-0.5 flex-1 mx-2 transition-colors duration-500" :class="{'bg-primary-light': currentStep >= 3, 'text-base-white/50': currentStep < 3}"></div>
                <span :class="{'text-primary-light': currentStep >= 3, 'text-base-white/50': currentStep < 3}">3. Signature</span>
            </div>
            ...
            <div x-show="!isSubmitting">
                ...
                {{-- STEP 2: BOOKING INFO & GROUP MEMBERS (All booking fields are now visible/required) --}}
