            ...
            <form action="{{ route('frontdesk.registrations.finalize', $registration) }}" method="POST">
                ...
                <div class="card shadow-lg">
                    <div class="card-header bg-warning text-dark">
                        <h4><i class="fas fa-check-double me-2"></i>Finalize Check-in for {{ $registration->full_name }}</h4>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <fieldset class="mb-4">
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    <p><strong>Lead Guest:</strong> {{ $registration->full_name }} ({{ $registration->title }})</p>
                                    <p><strong>Contact:</strong> {{ $registration->contact_number }} | {{ $registration->email }}</p>
                                    <p><strong>Stay:</strong> {{ $registration->check_in->format('M d, Y') }} to {{ $registration->check_out->format('M d, Y') }}</p>
                                </div>
                                <div class="col-md-4 text-center">
                                    <label>Guest Signature:</label>
                                    @if($registration->guest_signature)
                                        <img src="{{ Storage::url($registration->guest_signature) }}" alt="Guest Signature" class="img-fluid border rounded bg-white" style="max-height: 100px;">
                                    @else
                            ...
                            <div class="row bg-light p-3 rounded border">
                                ...
                                <div class="col-md-4 mb-3 d-flex align-items-end">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="bed_breakfast" value="1" id="bed_breakfast">
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" class="form-control" name="group_members[{{ $member->id }}][room_allocation]" placeholder="Room No." required>
                                                </td>
                                                <td>
                                                    <input type="number" step="0.01" class="form-control" name="group_members[{{ $member->id }}][room_rate]" placeholder="Rate" required>
                                                </td>
                                                <td>
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="group_members[{{ $member->id }}][bed_breakfast]" value="1">
                                                    </div>
                                                </td>
                                            </tr>
                                            @endforeach
                        ...
                        <hr class="my-4">
                        <button type="submit" class="btn btn-success btn-lg w-100">Complete Group Check-in</button>
                    </div>
                </div>
            </form>
        </div>
