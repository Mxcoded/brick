...
@section('content')
<div class="text-center mb-4">
    <h1 class="display-4 fw-bold">Our Menu - Table {{ $table }}</h1>
    <p class="lead text-muted">Explore our delicious offerings and place your order!</p>
</div>
    ...
    <div class="row">
        @foreach ($category->menuItems as $item)
            <div class="col-md-4 col-sm-6 mb-4">
                <div class="card h-100 shadow-sm border-0">
                    <div class="card-body">
                        <h3 class="card-title fw-bold">{{ $item->name }} <span class="float-end text-primary">&#8358; {{ number_format($item->price, 2) }}</span></h3>
                        <p class="card-text text-muted">{{ $item->description ?? 'No description available' }}</p>
                        <form action="{{ url('/table/' . $table . '/cart/add') }}" method="POST">
                            @csrf
                            <input type="hidden" name="item_id" value="{{ $item->id }}">
