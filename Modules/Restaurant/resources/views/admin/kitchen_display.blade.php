...
function playNewOrderSound() {
    try {
        if (!kdsAudio) {
            kdsAudio = new Audio('data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACAf39/f3+AgH+AgH+AgH+AgIC Af4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+Af39/f3+AgH+AgH+AgH+AgICA f4CAf3+AgH+Af3+AgH+Af3+AgH+A');
        }
        kdsAudio.play().catch(() => {});
    } catch (e) {}
}
...
async function refreshKDS() {
    try {
        ...
        if (preparing.length) {
            html += `
                <div class="col-12"><h5 class="text-primary fw-bold"><i class="bi bi-fire me-1"></i>Preparing</h5></div>
                ${preparing.map(order => renderOrderCard(order, 'primary')).join('')}
                <div class="col-12"><hr></div>`;
        }

        if (ready.length) {
            html += `
                <div class="col-12"><h5 class="text-success fw-bold"><i class="bi bi-check-circle me-1"></i>Ready to Serve</h5></div>
...
async function acceptOrder(orderId) {
    try {
        const res = await fetch('/restaurant-admin/kitchen/order/' + orderId + '/accept', {
            method: 'POST',
...
function renderOrderCard(order, color, showAccept) {
        ...
        <div class="col-md-4 col-lg-3">
            <div class="card border-${color} shadow-sm h-100">
                ...
                <div class="card-body py-2">
                    <small class="text-muted">${order.customer_name || 'Table ' + order.source_id} · ${timeAgo}m ago</small>
                    ...
                    ${showAccept ? `<button class="btn btn-success btn-sm w-100 mt-2 rounded-pill" onclick="acceptOrder(${order.id})"><i class="bi bi-check-lg me-1"></i>Accept Order</button>` : ''}
                </div>
            </div>
        </div>`;
}
