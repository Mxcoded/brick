...
@section('content')
    <div class="container-fluid content py-4" x-data="{ activeTab: 'all', categories: {{ json_encode($categories) }}, categoryNames: {{ json_encode($category_names) }} }">
        ...
        <div class="row g-4">
            ...
            <template x-else x-for="category in categories" :key="category.name">
                <template x-for="item in category.menu_items" :key="item.id">
                    <div class="col-md-6 offset-md-3 mb-3 mt-3" x-show="activeTab === 'all' || activeTab === category.name.toLowerCase()">
                                ...
                                <div class="d-flex">
                                    <div class="card-img-top text-center p-3" style="height: 200px; overflow: hidden;">
                                        <div class="image-wrapper" x-data="{ loaded: false }">
                                            ...
                                            <img :src="item.image ? '{{ asset('storage') }}/' + item.image : 'https://via.placeholder.com/150x150?text=No+Image'"
                                                 :alt="item.name"
                                    ...
                                    <div>
                                        ...
                                        <p class="fw-bold text-primary" x-text="'₦' + Number(item.price).toFixed(2)"></p>
                                    </div>
                                </div>
                                <div>
                                    <form @submit.prevent="$store.cart.add(item.id, instructions[item.id] || '', item.price, item.name)"
                                          method="POST"
                                          x-data="{ item_id: item.id, instructions: {}, price: item.price, name: item.name }">
                                        @csrf
        ...
        <div class="modal fade" id="cartModal" tabindex="-1" aria-labelledby="cartModalLabel" aria-hidden="true">
            <form class="modal-dialog" @submit.prevent="$store.cart.sendData()" method="POST" action="{{ route('restaurant.cart.order', $table) }}">
                @csrf
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                            ...
                            <div class="card p-3 mb-2 position-relative" :id="'cart-' + item.item_id" style="width: 100%;">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <p x-text="'Price: ₦' + Number(item.price).toFixed(2)"></p>
                                        <p x-text="'Instructions: ' + (item.instructions || 'None')"></p>
                                        <p x-text="'Total: ₦' + Number(item.total).toFixed(2)"></p>
                                    </div>
                                    <div class="d-flex align-items-center" style="height: 40px">
                                        <button type="button" @click="$store.cart.minusQuantity(item.item_id)" class="btn" style="background-color: #e4716e82; color: red">-</button>
                                        <span x-text="item.quantity" style="width: 40px; text-align: center; margin: 0 10px; padding: 5px"></span>
                                        <button type="button" @click="$store.cart.addQuantity(item.item_id)" class="btn" style="background-color: #e4716e82; color: red">+</button>
                                    </div>
                                </div>
                                <button type="button" @click="$store.cart.remove(item.item_id)" class="btn-close position-absolute" style="top: 10px; right: 10px"></button>
                            </div>
                        </template>
                        ...
                        <p>Total Price: <span x-text="'₦' + $store.cart.items.reduce((total, item) => total + Number(item.total), 0).toFixed(2)"></span></p>
                    </div>
        ...
        <div id="cart-number" class="position-fixed bottom-0 end-0 m-3 p-2 bg-danger text-white rounded-circle" style="display: none; width: 30px; height: 30px; text-align: center;" x-text="$store.cart.items.length"></div>
        ...
        <style>
            ...
            .fade-in[x-cloak], .fade-in[style*="display: none"] {
                opacity: 0 !important;
            }
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.store('cart', {
                ...
                add(item_id, instructions, price, name) {
                    if (this.items.some(item => item.item_id === item_id && item.instructions === instructions)) {
                ...
                sendData() {
                    ...
                    .then(data => {
                        window.location.href = "{{ route('restaurant.cart', $table) }}";
                    })
                    .catch(error => {
