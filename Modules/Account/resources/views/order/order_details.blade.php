...
@section('current-breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('account.order') }}">Orders</a></li>
    <li class="breadcrumb-item active" aria-current="page">{{ $order->order_number }}</li>
@endsection

@section('page-content')
    <div class="container-fluid my-4">
        <h1 class="mb-4 fw-bold text-dark">Order Details</h1>

        <!-- Main Employee Card -->
        <div class="card shadow-sm mb-5">
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center py-3">
                <h5 class="mb-0 fw-semibold  text-white">{{ $order->guest->full_name ?? 'N/A' }}</h5>
                <span class="badge bg-light text-dark px-2 py-1">{{ $order->guest->contact_number ?? 'N/A' }}</span>
                <span class="badge bg-light text-dark px-2 py-1">{{ $order->guest->company_name ?? 'N/A' }}</span>
            </div>
