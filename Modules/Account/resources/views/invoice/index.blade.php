                        ...
                        <tbody>
                            @foreach ($invoices as $invoice)
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-outline-success me-2 restock-btn" data-bs-toggle="modal" data-bs-target="#restockModal" data-item-id="{{ $invoice->id }}" data-item-description="{{ $invoice->invoice_number }}">
                                            <i class="fas fa-plus-circle"></i>
                                        </button>
                                        <a href="{{ route('account.invoice.show', $invoice->id) }}" class="btn btn-sm btn-outline-primary me-2"><i class="fas fa-eye"></i></a>
                                        <form action="{{ route('account.invoice.destroy', $invoice->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure you want to delete this invoice? This action cannot be undone.');"><i class="fas fa-trash"></i></button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
